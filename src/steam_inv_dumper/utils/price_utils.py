...
def get_steam_fees_object(price: decimal.Decimal) -> Dict[str, int]:
    ...
    nEstimatedAmountOfWalletFundsReceivedByOtherParty = round(
        int_price / (0.05 + 0.10 + 1), 0
    )
    ...
    intfees = {
        ...
        "money_to_ask": int(
            amount_to_send_desired_received_amt(fees["amount"] - fees["fees"])["amount"]
        ),
    }
    intfees["you_receive"] = int(
        intfees["money_to_ask"]
        - amount_to_send_desired_received_amt(fees["amount"] - fees["fees"])["fees"]
    )
